...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        next_value = [preorder(b) for b in branches(t)]
        result = [label(t)]
        for i in range(0, len(next_value)):
            result += next_value[i]
        return result
...
def sub_interval(x, y):
    ...
    and any value in y."""
    d1 = lower_bound(x) - upper_bound(y)
    d2 = upper_bound(x) - lower_bound(y)
    return interval(min(d1, d2), max(d1, d2))
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) != 0 and lower_bound(y) != 0, 'Can not divide a number by 0'
