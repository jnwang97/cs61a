...
def in_order_traversal(t):
    ...
    if t.is_leaf():
        ...
        for n in in_order_traversal(b1):
            yield n  # 生成器返回一个序列，递归的时候上一级的生成器要把下一级生成器返回的序列中的值拿出来再yield
        yield t.label
        for n in in_order_traversal(b2):
            yield n
...
def interleaved_sum(n, odd_term, even_term):
    ...
    def helper(i, iseven):
        if n == i:
            return term[iseven](i)  # base case在n这一侧，妙！
        else:
            return term[iseven](i) + helper(i+1, 1-iseven)
    return helper(1, 0)
...
def mutate_reverse(link):
    ...
    while temp is not Link.empty:
        elements.append(temp.first)
        temp = temp.rest
    elements_reversed = elements[::-1]
    while link is not Link.empty:
        link.first = elements_reversed[0]
        elements_reversed = elements_reversed[1:]
        link = link.rest
