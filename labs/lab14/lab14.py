...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            match, step = helper_align(skeleton_idx + 1, code_idx + 1)
            edits = code_char
            possibilities.append((edits + match, step))
        # Insert
        edits = "+[" + code_char + "]"
        match, step = helper_align(skeleton_idx, code_idx + 1)
        possibilities.append((edits + match, step + 1))
        # Delete
        edits = "-[" + skel_char + "]"
        match, step = helper_align(skeleton_idx + 1, code_idx)
        possibilities.append((edits + match, step + 1))
...
def num_splits(s, d):
    ...
    def helper(s, sum1, sum2):
        if not s:
            ...
            return helper(s[1:], s[0]+sum1, sum2) + helper(s[1:], sum1, s[0]+sum2)  # 妙！！
...
def insert(link, value, index):
    ...
    if link is not Link.empty and index == 0:
        # link = Link(value, link) , here link is a local varaible
        tmp = link.first
        link.first = value
